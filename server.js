...
const { updateLocaleFile, ___ } = require('./helpers/helper')
  ...
  if (req.headers['content-type'] !== 'application/json') {
    ...
    const siteTitle = pathSegments.reverse().reduce((acc, pathSegment) => {
      if (!pathSegment.startsWith("?") && pathSegment !== req.session.locale && pathSegment !== 'pages') {
        const pathSegementWithoutGermanAutogeneratedSlug = pathSegment.replace(/-2/g, '')
        ...
        const translagedPathSegement = ___(upperCasePathSegment, req.session.locale)
