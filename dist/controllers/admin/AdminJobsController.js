    ...
    try {
        return new Promise(async (resolve, reject) => {
            ...
            for (let queryLang of queryLangs) {
                const url = `https://dci-jobs.personio.de/xml${queryLang === "en" ? `?language=en` : ``}`;
            ...
            for (let job of xml) {
                try {
                    ...
                    if (existingJob) {
                        await Job.update({ personio_id: job.id }, { "$push": { "description": job.jobDescriptions.jobDescription } }).exec();
                    }
                    else {
