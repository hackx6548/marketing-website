...
exports.resizeImages = async (request, response, next) => {
    ...
    for await (const singleFile of Object.values(request.files)) {
        ...
        request.body[singleFile[0].fieldname] = `${singleFile[0].filename}.${extension}`;
        try {
            const image = await jimp.read(singleFile[0].path);
            // await image.cover(350, 180);
            await image.write(path.resolve(process.env.IMAGE_UPLOAD_DIR, request.body[singleFile[0].fieldname]));
...
exports.updateProfile = async (request, response) => {
    await User.findOneAndUpdate({ _id: request.story.slug }, request.body, {
        new: true,
        runValidators: true
    }).exec();
...
//# sourceMappingURL=AdminStoriesController.js.map