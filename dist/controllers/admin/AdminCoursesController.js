    ...
    for await (const singleFile of Object.values(request.files)) {
        ...
        try {
            if (singleFile[0].mimetype === "image/svg+xml" || singleFile[0].mimetype === "application/pdf") {
                ...
                fs.writeFileSync(path.resolve(process.env.IMAGE_UPLOAD_DIR, filename), fileObject);
            }
            ...
            items.map((_, i) => {
                titles.map(title => {
                    ...
                    if (req.body[`${title.reqChild}${i + 1}`] === "") {
                        ...
                        course[model][i][title.dbChild] = req[model == "archivements" && title.dbChild == "icon"
                            ? "files"
                            : "body"][`${title.reqChild}${i + 1}`]
