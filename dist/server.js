...
app.use(function (req, res, next) {
    ...
    if (!!query && Object.keys(query).length > 0 && (req.session.utmParams === undefined || req.session.utmParams.length === 0)) {
        ...
        const mappedParams = { ...Object.keys(query).map(paramKey => Array.isArray(query[paramKey]) ? ({ [paramKey]: [...new Set(query[paramKey])] }) : ({ [paramKey]: query[paramKey] })) };
...
app.use(async (req, res, next) => {
    ...
    if (redisClient !== undefined && process.env.USE_REDIS === 'true') {
        try {
    ...
    if (navData === null) {
        ...
        try {
            if (process.env.USE_REDIS === 'true' && undefined) {
...
if (process.env.CRONINTERVAL) {
    ...
    cron.schedule(process.env.CRONINTERVAL, () => {
